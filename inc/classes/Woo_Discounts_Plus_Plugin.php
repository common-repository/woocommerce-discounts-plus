...
//				add_action( 'woocommerce_plus_discount_settings', array( $this, 'add_settings_fields' ), 10 );
			...
			public function wpdp_woocommerce_cart_totals_before_shipping(){
				...
				echo '<tr class="wpdp-cart-discount ctbs">
			...
			public function wpdp_woocommerce_cart_totals_before_order_total(){
				...
				if($this->gj_logic()){
					...
					foreach($this->discount_love as $item_id=>$item_arr){
						...
						//if($item_arr['coeff']>$coeff && $item_arr['coeff']<1){
							$coeff = $item_arr['coeff'];
						//}
						//pree($item_arr['orig_price'].' * '.$coeff);
						...
						if(array_key_exists($item_id, $d_items)){

							$d_total_discount = ($d_items[$item_id]-($d_items[$item_id]*$coeff));
							
							$d_total_discounts[] = $d_total_discount;
					...
					if($is_flat){
						...
						//pree($d_items);
						$d_total_amount = array_sum($d_items);
						//pree('('.$d_total_amount.'-('.$d_total_amount.'*'.$coeff.'))');
						$d_total_discount = array_sum($d_total_discounts);

						//pree($d_total_discount);pree($d_total_amount);

						$dpa = $d_total_amount?($d_total_discount/$d_total_amount):0;
						//pree($dpa);
						$d_percentage = ($dpa!=1?' ('.round($dpa*100).'%)':'');
					}
				...
				if($d_percentage){
					...
					echo '<tr class="wpdp-cart-discount ctbot">
						<th>'.get_wcdp_discount_label().' '.$d_percentage.'</th>
			...
			protected function get_discounted_coeff( $product_id, $quantity, $composite_qty = array(), $item_data=array() ) {
			
				global $wdp_price_num_decimals, $s2_enabled, $s2_discounts;
				//pre($this->sale_applied());
				//pree($product_id. ' - ' .$quantity);pree($composite_qty);//exit;
				//pree($item_data['quantity']);
				$plus_discount_enabled = get_post_meta($product_id, 'plus_discount_enabled', true);
				
				$qty_total_arr = $this->qty_total_arr;
				
				switch($plus_discount_enabled){
					case 'category_based':
						$terms = get_the_terms( $product_id, 'product_cat' );
						//pree($terms);
						$cat_id = get_post_meta($product_id, 'dc_cat_id', true);
						//pree($cat_id);
						//pree($qty_total_arr);
						//pree(array_key_exists($cat_id, $qty_total_arr));
						if(is_array($qty_total_arr) && array_key_exists($cat_id, $qty_total_arr)){
							$quantity = $qty_total_arr[$cat_id];
						}else{
							$quantity = 1;
						}
						//pree($quantity);
...
//				pree($plus_discount_type_globally);exit;
				...
				if($s2_enabled && $s2_discounts){
					...
					if( $fp == 'flat' ){
						...
						//pree($quantity); pree($composite_qty);
						$qty_considered = 0;
						if($this->gj_logic()){
							...
							if(!$dkey_guess){
								...
								//$qty_considered = ($qty_considered<$item_data['quantity']?$qty_considered:$item_data['quantity']);
								//pree($qty_considered);
							}
						}else{
							//pree($qty_considered.' = '.$quantity);
							$qty_considered = $quantity;
						}

						//pree($q);
						$dkey = array_search($qty_considered, $q);
						//pree($qty_considered.' - '.$dkey);
						...
						if(isset($d[$dkey])){
							...
							//pree($qty_considered. ' - ' .$dkey. ' - ' .$d[$dkey]. ' - '. $fm );
...
//				pree($this->discount_love[$product_id]);
			...
			public function filter_item_price( $price, $values ) {
				...
				$this->gather_discount_love($values);
...
//			pree($newsubtotal);exit;
			...
			public function filter_item_price_single( $price_html, $product ) {
				...
				if(!is_admin() && ((is_product() && $wdp_new_price_sp) || (is_shop() && $wdp_new_price_shop))){
					...
					if(method_exists($woocommerce->cart, 'get_cart')){
			 ...
			 */
			protected function gather_discount_love($item_data=array()) {
				
				global $woocommerce, $wdp_discount_types, $woocommerce_variations_separate, $product_variations_qty;
				
				$item_key = ((is_array($item_data) && !empty($item_data) && isset($item_data['key']))?$item_data['key']:'');
				...
				if ( sizeof( $cart->cart_contents ) > 0 ) {
					...
					if($item_key && array_key_exists($item_key, $cart->cart_contents)){
						$cart_contents = array($item_key=>$cart_contents[$item_key]);
					}
					...
					$q = 0;
					foreach ( $cart_contents as $cart_item_key => $values ) { $q++;
						...
						if($this->gj_logic()){
							...
							switch($plus_discount_type){
								...
								case 'quantity':
									...
									//pree($this->get_actual_id($_product));
									//pree($this->get_actual_id($_product).' - '.$this->qty_total);
									//pree($values);
									$coeff = $this->get_discounted_coeff( $this->get_actual_id($_product), $this->qty_total, array(), $values );
								...
								case 'weight':
									...
									//pre($qty_updated);
									$coeff = $this->get_discounted_coeff( $this->get_actual_id($_product), $qty_updated, array(), $values );
									$this->discount_love[$ac_id]['coeff'] = $coeff;
								break;
							}
							//$_product->get_id()
							...
							$coeff = $this->get_discounted_coeff( $this->get_actual_id($_product), $quantity, $product_variations_qty, $values); //$_product->get_id();
							//pree($coeff);

							$this->discount_love[$ac_id]['coeff'] = $coeff;
						}

						//pree($ac_id.' = '.$coeff.' = '.$_product->get_price());
						//pree($_product);
